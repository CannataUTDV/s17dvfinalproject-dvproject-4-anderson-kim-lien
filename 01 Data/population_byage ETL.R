...
file_path = "../../CSVs/PreETL_population_byage.csv"
population_byage <- readr::read_csv(file_path)
names(population_byage)

df <- plyr::rename(population_byage, c("table"="tbl")) # table is a reserved word in Oracle so rename it to tbl.
names(df)
str(df) # Uncomment this line and  run just the lines to here to get column types to use for getting the list of measures.

dimensions <- c("AreaName")
