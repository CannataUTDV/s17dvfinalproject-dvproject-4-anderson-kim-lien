...
file_path = "../../CSVs/PreETL_adolescentobese.csv"
adolescentobese <- readr::read_csv(file_path)
names(adolescentobese)

df <- plyr::rename(adolescentobese, c("table"="tbl")) # table is a reserved word in Oracle so rename it to tbl.
names(df)
names(df)[1]<-"Location"
names(df)[2]<-"Value"
