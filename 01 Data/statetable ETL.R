...
file_path = "../../CSVs/PreETL_statetable.csv"
statetable <- readr::read_csv(file_path)
names(statetable)

df <- plyr::rename(statetable, c("table"="tbl")) # table is a reserved word in Oracle so rename it to tbl.
names(df)
str(df) # Uncomment this line and  run just the lines to here to get column types to use for getting the list of measures.

dimensions <- c("name","abbreviation","country","type","status","occupied","notes","assoc_press","standard_federal_region","census_region_name","census_division_name")
