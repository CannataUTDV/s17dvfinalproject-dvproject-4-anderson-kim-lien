...
file_path = "../../CSVs/PreETL_RiskFactors.csv"
riskfactors <- readr::read_csv(file_path)
names(riskfactors)

df <- plyr::rename(riskfactors, c("table"="tbl")) # table is a reserved word in Oracle so rename it to tbl.
names(df)
str(df) # Uncomment this line and  run just the lines to here to get column types to use for getting the list of measures.

dimensions <- c("State_FIPS_Code","County_FIPS_Code","CHSI_County_Name","CHSI_State_Name","CHSI_State_Abbr", "Strata_ID_Number")
